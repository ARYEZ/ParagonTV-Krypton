...
def log(msg):
    ...
    if KODI_MODE:
        xbmc.log("[TV Genre Consistency] {}".format(msg), xbmc.LOGNOTICE)
    else:
...
def scan_tv_folders(base_path, progress_dialog=None):
    ...
    if progress_dialog:
        progress_dialog.update(0, "Scanning TV library...", "Finding show folders...", "")
    ...
    try:
        ...
        for idx, folder_name in enumerate(dirs):
            if progress_dialog:
                ...
                progress_dialog.update(percent, "Scanning TV library...", "Checking: {}".format(folder_name), "")
...
def check_tv_genre_consistency(base_path):
    ...
    # Write report
    if KODI_MODE:
        report_path = xbmc.translatePath('special://temp/tv_genre_consistency_report.txt')
    else:
