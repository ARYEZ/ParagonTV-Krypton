...
class AdvancedConfig(xbmcgui.WindowXMLDialog):
    ...
    def onAction(self, act):
        ...
        if focusid >= 160:
            self.getControl(focusid).setLabel(
                self.ruleList[self.selectedRuleIndex].onAction(
        ...
        if action in ACTION_PREVIOUS_MENU:
            ...
            if focusid > 119 and focusid < (120 + RULES_PER_PAGE):
                ...
                if (focusid == (119 + RULES_PER_PAGE)) and (
                    self.lineSelected == (RULES_PER_PAGE - 1)
                ):
                    ...
                    if self.listOffset != curoffset:
                        xbmc.executebuiltin(
                            "Control.SetFocus(" + str(119 + RULES_PER_PAGE) + ")"
                        )
                else:
    ...
    def setupOptions(self):
        ...
        for i in range(4):
            if i < (optcount - (self.optionRowOffset * 2)):
                ...
                self.getControl(i + 150).setLabel(
                    self.ruleList[self.selectedRuleIndex].getOptionLabel(
                        i + (self.optionRowOffset * 2)
                    )
                )
                self.getControl(i + 160).setVisible(True)
                self.getControl(i + 160).setEnabled(True)
                self.getControl(i + 160).setLabel(
                    self.ruleList[self.selectedRuleIndex].getOptionValue(
                        i + (self.optionRowOffset * 2)
                    )
                )
            else:
        ...
        if self.selectedRuleIndex >= 0:
            ...
            for i in range(self.allRules.getRuleCount()):
                if self.allRules.getRule(i).getId() == curid:
                    self.ruleList[self.selectedRuleIndex] = self.allRules.getRule(
                        i - 1
                    ).copy()
                    break
    ...
    def setRuleControls(self, listindex):
        ...
        self.getControl(130).setLabel(
            "Rule " + str(self.selectedRuleIndex + 1) + " Configuration"
        )
        ...
        self.getControl(131).setLabel(
            "<-"
            + spacesstr
            + self.getRuleName(self.selectedRuleIndex)
            + spacesstr
            + "->"
        )
        self.setupOptions()
