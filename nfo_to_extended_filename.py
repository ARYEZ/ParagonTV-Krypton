#!/usr/bin/env python3
"""
NFO to Extended Filename Converter
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("nfo_renamer.log"),
...
HOLIDAY_KEYWORDS = {
    ...
    'Halloween': ['halloween', 'trick or treat', 'trick-or-treat', 'spooky', 'october 31', 'all hallows']
}
...
# NEW: Pattern to detect if a file is already in the extended format
# Format: SSxEE - Episode Title - Show Title - Genre - Resolution - Audio Channels - Audio Codec - Holiday.ext
EXTENDED_FORMAT_PATTERN = re.compile(r'^\d+x\d+ - .+ - .+ - .+ - \d+ - \d+ - [A-Za-z0-9-]+ - [A-Za-z]+')

# NEW: Function to sanitize filenames for Windows compatibility
...
# NEW: Function to check if a file is already in the extended format
    ...
    for holiday, keywords in HOLIDAY_KEYWORDS.items():
        for keyword in keywords:
            if keyword in plot_lower:
                logger.info(f"Detected {holiday} episode based on keyword '{keyword}'")
                return holiday
...
def parse_nfo_file(nfo_path):
    ...
    try:
        ...
        logger.error(f"Error parsing XML in {nfo_path}: {e}")
        return None
    except Exception as e:
        logger.error(f"Error processing {nfo_path}: {e}")
        return None
...
def get_tvshow_metadata(episode_nfo_path):
    ...
    if parent_dir in tvshow_metadata_cache:
        logger.debug(f"Using cached tvshow metadata for {parent_dir}")
    ...
    if not os.path.exists(tvshow_nfo_path):
        logger.warning(f"tvshow.nfo not found for {episode_nfo_path}")
    ...
    try:
        ...
        if genre_elem is not None and genre_elem.text:
            ...
            logger.info(f"Found genre '{tvshow_metadata['genre']}' in tvshow.nfo for {parent_dir}")
        ...
        if originaltitle_elem is not None and originaltitle_elem.text:
            ...
            logger.info(f"Found show title '{tvshow_metadata['showtitle']}' from originaltitle tag in tvshow.nfo")
        else:
            # If not found, try title tag
            title_elem = root.find('.//title')
            if title_elem is not None and title_elem.text:
                tvshow_metadata['showtitle'] = title_elem.text.strip()
                logger.info(f"Found show title '{tvshow_metadata['showtitle']}' from title tag in tvshow.nfo")
        ...
        logger.error(f"Error parsing XML in tvshow.nfo: {e}")
        return {'genre': None, 'showtitle': None}
    except Exception as e:
        logger.error(f"Error processing tvshow.nfo: {e}")
        return {'genre': None, 'showtitle': None}
...
def create_extended_filename(metadata, original_ext):
    ...
    filename = f"{season:02d}x{episode:02d} - {title} - {showtitle} - {genre} - {resolution} - {audio_channels} - {audio_codec} - {holiday}{original_ext}"
...
def rename_files(directory, dry_run=False, recursive=False):
    ...
    stats = {
        ...
        'errors': 0,
        'skipped': 0,
        'already_extended': 0,  # NEW: count files already in extended format
        'genre_from_tvshow': 0,
    ...
    for filename in os.listdir(directory):
        ...
        if os.path.isdir(file_path) and recursive:
            ...
            sub_stats = rename_files(file_path, dry_run, recursive)
            if sub_stats:
                # Merge statistics
                for key in stats:
                    if key in sub_stats:
        ...
        if filename.lower().endswith('.nfo') and filename.lower() != 'tvshow.nfo':
            ...
            if not video_file:
                logger.warning(f"No matching video file found for NFO: {filename}")
                stats['skipped'] += 1
                continue
            
            # NEW: Check if video file is already in extended format
            if is_already_extended_format(video_file):
                logger.info(f"File already in extended format: {video_file}")
            ...
            if not metadata:
                logger.error(f"Failed to parse NFO: {filename}")
                stats['errors'] += 1
                continue
            ...
            if need_tvshow_metadata:
                ...
                if not metadata['genre'] and tvshow_metadata['genre']:
                    ...
                    logger.info(f"Using genre from tvshow.nfo: {tvshow_metadata['genre']}")
                ...
                if not metadata['showtitle'] and tvshow_metadata['showtitle']:
                    ...
                    logger.info(f"Using show title from tvshow.nfo: {tvshow_metadata['showtitle']}")
            ...
            if not metadata['audio_channels'] or not metadata['audio_codec']:
                ...
                channels_match = re.search(r'(\d+\.\d+)ch|(\d+)ch|(\d+\.\d+)|(\d+)channels', base_name.lower())
                if channels_match:
            ...
            if new_video_name == video_file and new_nfo_name == filename:
                logger.info(f"Files already have correct naming: {filename}")
                stats['skipped'] += 1
                continue
                
            # Log the rename operation
            logger.info(f"Renaming:\n  {filename} -> {new_nfo_name}\n  {video_file} -> {new_video_name}")
    ...
    logger.info(f"Directory {directory} - Processed: {stats['processed']}, "
                f"Renamed: {stats['renamed']}, Errors: {stats['errors']}, "
                f"Skipped: {stats['skipped']}, Already Extended: {stats['already_extended']}, "
                f"Genre from tvshow.nfo: {stats['genre_from_tvshow']}, "
                f"Show title from tvshow.nfo: {stats['showtitle_from_tvshow']}, "
                f"Holiday episodes - Christmas: {stats['christmas_episodes']}, "
                f"Thanksgiving: {stats['thanksgiving_episodes']}, "
                f"Halloween: {stats['halloween_episodes']}")
...
def main():
    ...
    parser.add_argument(
        "directory", 
        help="Directory containing NFO and video files to process"
    )
    ...
    print(f"NFO to Extended Filename Converter")
    ...
    try:
        stats = rename_files(args.directory, args.dry_run, args.recursive)
        if stats:
            print("\nOperation completed successfully.")
            print(f"Files processed: {stats['processed']}")
            print(f"Files renamed: {stats['renamed']}")
            print(f"Files skipped: {stats['skipped']}")
            print(f"Files already in extended format: {stats['already_extended']}")
            print(f"Errors encountered: {stats['errors']}")
            print(f"Genre from tvshow.nfo: {stats['genre_from_tvshow']}")
            print(f"Show title from tvshow.nfo: {stats['showtitle_from_tvshow']}")
            print("\nHoliday episodes found:")
            print(f"  Christmas: {stats['christmas_episodes']}")
            print(f"  Thanksgiving: {stats['thanksgiving_episodes']}")
            print(f"  Halloween: {stats['halloween_episodes']}")
    except Exception as e:
